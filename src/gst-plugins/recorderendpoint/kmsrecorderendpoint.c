...
#define PLUGIN_NAME "lolrecorderendpoint"
...
} LolKmsRecorderEndpointTransition;
...
struct _LolKmsRecorderEndpointPrivate
{
  ...
  LolKmsRecorderEndpointTransition transition;
...
G_DEFINE_TYPE_WITH_CODE (LolKmsRecorderEndpoint, kms_recorder_endpoint,
...
static gchar *
kms_element_get_padname_from_id (LolKmsRecorderEndpoint * self, const gchar * id)
{
...
static void
kms_recorder_endpoint_change_state (LolKmsRecorderEndpoint * self,
    LolKmsRecorderEndpointTransition transition)
{
...
static void
kms_recorder_endpoint_update_internal_state (LolKmsRecorderEndpoint * self,
    KmsUriEndpointState state)
{
  if (state == KMS_URI_ENDPOINT_STATE_START) {
...
static void
kms_recorder_endpoint_sync_state_changed (LolKmsRecorderEndpoint * self,
    KmsUriEndpointState state)
{
...
static void
kms_recorder_endpoint_async_state_changed (LolKmsRecorderEndpoint * self,
    KmsUriEndpointState state)
{
 ...
 */
static guint
kms_recorder_endpoint_send_eos_to_appsrcs (LolKmsRecorderEndpoint * self)
{
  guint size;
...
{
  LolKmsRecorderEndpoint *self = KMS_RECORDER_ENDPOINT (object);
...
static void
kms_recorder_endpoint_release_pending_requests (LolKmsRecorderEndpoint * self)
{
...
static void
kms_recorder_generate_pads (LolKmsRecorderEndpoint * self)
{
...
static void
kms_recorder_endpoint_remove_pads (LolKmsRecorderEndpoint * self)
{
  if (self->priv->generate_pads) {
    return;
  }
...
static void
kms_recorder_endpoint_create_parent_directories (LolKmsRecorderEndpoint * self)
{
...
{
  LolKmsRecorderEndpoint *self = KMS_RECORDER_ENDPOINT (obj);
  KmsUriEndpointState state;
...
configure_pipeline_capabilities (GstPad * pad, GstPadProbeInfo * info,
    gpointer user_data)
{
  LolKmsRecorderEndpoint *self = KMS_RECORDER_ENDPOINT (user_data);
...
{
  LolKmsRecorderEndpoint *self = KMS_RECORDER_ENDPOINT (pad->linkdata);
...
static void
kms_recorder_release_pending_pad (gchar * id, LolKmsRecorderEndpoint * self)
{
...
{
  LolKmsRecorderEndpoint *self = KMS_RECORDER_ENDPOINT (pad->unlinkdata);
  gchar *id = NULL;
  GstElement *appsrc;
...
static void
kms_recorder_endpoint_add_appsink (LolKmsRecorderEndpoint * self,
...
static void
kms_recorder_endpoint_update_media_stats (LolKmsRecorderEndpoint * self)
{
...
{
  LolKmsRecorderEndpoint *recorder = KMS_RECORDER_ENDPOINT (user_data);
  gboolean stop = FALSE;
...
static void
kms_recorder_endpoint_create_base_media_muxer (LolKmsRecorderEndpoint * self)
{
  KmsBaseMediaMuxer *mux;
...
static void
kms_recorder_endpoint_new_media_muxer (LolKmsRecorderEndpoint * self)
{
  GstBus *bus;
...
kms_recording_profile_create_raw_audio_profile ()
{
  GstEncodingContainerProfile *cprof;
  GstCaps *vc;
  GstCaps *pc;

  pc = gst_caps_from_string ("audio/x-raw");
  cprof = gst_encoding_container_profile_new ("audio", NULL, pc, NULL);
  gst_caps_unref (pc);

  vc = gst_caps_from_string ("audio/x-raw,channels=1,rate=16000,format=S16LE");

  gst_encoding_container_profile_add_profile (cprof, (GstEncodingProfile *)
      gst_encoding_audio_profile_new (vc, NULL, NULL, 0));
...
static GstCaps *
kms_recorder_endpoint_get_caps_from_profile (LolKmsRecorderEndpoint * self,
    KmsElementPadType type)
{
  GstEncodingContainerProfile *cprof;
...
kms_recorder_endpoint_query_caps (KmsElement * element, GstPad * pad,
    GstQuery * query)
{
  LolKmsRecorderEndpoint *self = KMS_RECORDER_ENDPOINT (element);
...
static GstStructure *
kms_element_get_e2e_latency_stats (LolKmsRecorderEndpoint * self, gchar * selector)
{
  gpointer key, value;
  GHashTableIter iter;
  GstStructure *stats;
...
static void
kms_recorder_endpoint_class_init (LolKmsRecorderEndpointClass * klass)
{
  ...
  g_type_class_add_private (klass, sizeof (LolKmsRecorderEndpointPrivate));
}
...
static ErrorData *
create_error_data (LolKmsRecorderEndpoint * self, GstMessage * message)
{
  ErrorData *data;
...
{
  LolKmsRecorderEndpoint *self = KMS_RECORDER_ENDPOINT (data);
...
static void
kms_recorder_endpoint_init (LolKmsRecorderEndpoint * self)
{
  GError *err = NULL;
