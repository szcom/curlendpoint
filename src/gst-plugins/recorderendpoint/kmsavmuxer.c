...
kms_av_muxer_create_muxer (KmsAVMuxer * self)
{
  switch (KMS_BASE_MEDIA_MUXER_GET_PROFILE (self)) {
    ...
    case KMS_RECORDING_PROFILE_MP4_AUDIO_ONLY:{
      // ZZZ GstElement *mux = gst_element_factory_make ("mp4mux", NULL);
      GstElement *mux = gst_element_factory_make ("identity", NULL);
      //g_object_set(mux, "caps", "caps=audio/x-raw,channels=1,rate=16000,format=S16LE", NULL);
      ...
      if ((gst_element_factory_get_element_type (sink_factory) !=
        ...
        // ZZZ g_object_set (mux, "faststart", TRUE, NULL);
      }
